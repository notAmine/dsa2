...
def get_model(   original_dim,class_num,intermediate_dim,intermediate_dim_2,latent_dim,Lambda1,batch_size,Lambda2,Alpha,):
    """
...
def fit(data_pars=None, compute_pars=None, out_pars=None, **kw):
    ...
    # dummy_train = np.ones((Xtrain_tuple.shape[0], 1))
...
def get_dataset(data_pars=None, task_type="train", **kw):
    ...
    """
    # log(data_pars)
    if data_pars['dataset_name'] == 'correlation' :
       x_train = get_mydata_correl(data_pars)
       ytrain = np.ones((x_train.shape[0], 1))
...
def test():
    ...
    X = get_mydata_correl(adata_pars)
    ...
    predict(Xpred=Xpred, data_pars= data_pars, compute_pars= compute_pars)
