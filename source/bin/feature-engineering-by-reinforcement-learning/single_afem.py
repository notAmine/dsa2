...
def simulate(inp):
    ...
    # loading the trained weights if given as argument
    if load:
    ...
    total_reward = 0
    env.reset()
    # iterating over the staespace by taking the best action
                ...
                for fid in tqdm(range(dataset.shape[1]-1),total = dataset.shape[1]-1):
                    ...
                    for i in tqdm(range(num_local_epochs), total = num_local_epochs):
                        ...
                        # for n in range(num_episodes):
                            ...
                            #print(total_reward)
                        mean_loss=[]
                        # optimizating the iterations and printing the loss after evry training step
