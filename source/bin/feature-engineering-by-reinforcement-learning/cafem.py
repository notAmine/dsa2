...
class Tasks():
    def __init__(self,datasetids,buffer_size=100):
        ### Initializing log files for each dataset
        self.datasets = {}
        self.tasks = []
        if os.path.isfile('data/log.csv'):
            ...
            f.close()            
    def sample(self,n):
        ### Returning n samples from list of tasks at random
...
def generate_trajectories(task):
    ...
    #print('loading')
    # creating deep neural network with specific depth based on arguments
    ...
    saver = tf.train.Saver() #saving trained weights


    for n in range(num_episodes):
        # get the maximum reward for each action for each dataset
        total_reward = 0
        env.reset()
...
def main():
    # creating deep q network with specific output size and input size based on arguments
    ...
    # applying tqdm for num_epochs - training started
    for g in tqdm(range(num_epochs), total = num_epochs):
        ...
        # storing states and actions on each iteration
        for task in sample_tasks:
            task_buff                = task[2]
        ...
        # storing losses in the log file
